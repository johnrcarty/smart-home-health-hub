...
def get_serial_log():
    """Return the last raw serial lines as a list (most recent last)."""
    return list(serial_log)


def broadcast_serial_raw(raw_line: str):
    """Append a raw serial line to buffer and broadcast to all WS clients."""
    if raw_line is None:
        return
    try:
        serial_log.append(raw_line)
        if not event_loop:
            return
        message = {"type": "serial_raw", "line": raw_line}
        for ws in list(websocket_clients):
            try:
                asyncio.run_coroutine_threadsafe(ws.send_json(message), event_loop)
            except Exception as e:
                print(f"[state_manager] Failed to send serial_raw to websocket: {e}")
                websocket_clients.discard(ws)
    except Exception as e:
        print(f"[state_manager] Error broadcasting serial raw: {e}")

def save_pulse_ox_batch():
    """
    Save batched pulse ox data to database
    """
    global pulse_ox_batch_buffer, batch_start_time
    
    if not pulse_ox_batch_buffer:
        return
    
    print(f"[state_manager] Saving batch of {len(pulse_ox_batch_buffer)} pulse ox readings")
    
    try:
        with get_db_session() as db:
            success = save_pulse_ox_batch(db, pulse_ox_batch_buffer)
            if success:
                print(f"[state_manager] Successfully saved {len(pulse_ox_batch_buffer)} pulse ox readings")
            else:
                print(f"[state_manager] Failed to save pulse ox batch")
    except Exception as e:
        print(f"[state_manager] Error saving pulse ox batch: {e}")
    finally:
        # Clear the batch buffer
        pulse_ox_batch_buffer = []
        batch_start_time = None


def should_save_batch():
    """
    ...
    return (time.time() - batch_start_time) >= BATCH_DURATION_SECONDS